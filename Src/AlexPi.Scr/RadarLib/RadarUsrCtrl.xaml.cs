...
public partial class RadarUsrCtrl
{
  ...
  // [Obsolete] :why Copilot decides to mark it such? //todo: why VS made it obsolete????????
  public RadarUsrCtrl()
  {
    InitializeComponent();
  ...
  async Task showPicX(int idx)
  {
    try
    {
      ...
      LTitle.Text = string.Format("{0}  {1,2}/{2}  ({3}-{4}={5:N1})   {6}",
        ...
        _radarPicCollector.Pics[_curPicIdx].ImageTime.ToString("ddd HH:mm"),
        _radarPicCollector.Pics[^1].ImageTime.ToString("HH:mm"),
        new TimeSpan(_radarPicCollector.Pics[^1].ImageTime.Ticks - _radarPicCollector.Pics[_curPicIdx].ImageTime.Ticks).TotalHours,
  ...
  public async Task OnKeyDown__Async(Key key)
  {
    switch (key)
    {
      ...
      case Key.Delete: _ = DeleteOldImages(); break;
    }
  }
  ...
  void fetchFromWebBegin()
  {
    ...
    for (var i = 0; i < _radarPicCollector.StationCount - 1; i++) { _ = new IntArgDelegate(fetchFromWebBlockingCall_FetchRad).BeginInvoke(i, null, null); }
