...
public partial class ControlPanel : TopmostUnCloseableWindow
{
  ...
  {
    await App.SpeakAsync($"Key: {e.Key}.", ignoreBann: true);
    switch (e.Key)
    {
      //case Key.F12:
      ...
      case Key.None: await App.SpeakAsync($"Toggled.", ignoreBann: true); break;

      default:/**/ await App.SpeakAsync($"Handled set to false!", ignoreBann: true); return;
    }

    e.Handled = true;
    await App.SpeakAsync($"Handled set to true.", ignoreBann: true);
  }
