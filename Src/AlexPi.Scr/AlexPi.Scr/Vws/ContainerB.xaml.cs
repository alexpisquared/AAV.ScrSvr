...
{
  PerformanceCounter
          //_perfCountAA = new PerformanceCounter("Memory", "Available MBytes"), // new PerformanceCounter(categoryName: "Process", counterName: "Bytes", instanceName: "AlexPi.Scr.scr"),
          //_perfCountEx = new PerformanceCounter(categoryName: "Process", counterName: "Bytes", instanceName: "Explorer"),
  ...
  public ContainerB(AlexPi.Scr.Logic.GlobalEventHandler globalEventHandler) : base(globalEventHandler)
  {
    ...
    _ = Task<PerformanceCounter>.Run(() => new PerformanceCounter("Processor", "% Processor Time", "_Total")).ContinueWith(_ => // new (...) takes forever => using Task to unfreeze (Aug 2019)
    {
  ...
  void measure_Cpu_for_arrows_only()
  {
    try
    {
      ...
      catch (Exception ex) { _ = ex.Log("1111"); }
      try
      {
        gaugeTorCPU.MiddlValAnim = v;
      }
      catch (Exception ex) { _ = ex.Log("2222-Height made Abs()."); }
      try
      {
    ...
    catch (Exception ex) { _ = ex.Log($"_perfCounCPU.InstanceName: {_perfCounCPU.InstanceName}"); }
  }
}