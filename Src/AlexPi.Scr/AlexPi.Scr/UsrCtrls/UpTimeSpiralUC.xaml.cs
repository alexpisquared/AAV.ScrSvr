...
public partial class UpTimeSpiralUC : UserControl
{
  ...
  async void onLoaded(object s, RoutedEventArgs e)
  {
    ...
    if (DateTime.Today == TrgtDate) _ = new DispatcherTimer(TimeSpan.FromSeconds(10), DispatcherPriority.Background, new EventHandler((s_, e_) => updateTimes(DateTime.Now)), Dispatcher.CurrentDispatcher); //tu: one-line timer
  }
  ...
  void drawUpTimeSegments(DateTime trgDate)
  {
    ...
    _ = Task.Run(() => EvLogHelper.GetAllUpDnEvents(timeA, timeB)).ContinueWith(_ =>
    {
      var eois = _.Result;
      if (eois.Count < 1)
      ...
      else
      {
        ...
        else if (prevEoiF is ((int)EvOfIntFlag.ScreenSaverrDn) or ((int)EvOfIntFlag.BootAndWakeUps))
