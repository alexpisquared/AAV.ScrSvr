...
[Obsolete(@"Instead use  C:\g\Radar\Src\Radar\Logic\PicMea.cs")]
public static class PicMea
{
  public static double CalcMphInTheArea(System.Drawing.Bitmap bmp, DateTime imgTime, int radiusInPixels = 40)
  {
    ...
    try
    {
      ...
      var bkgr = System.Drawing.Color.FromArgb(255, 153, 153, 102);              //var hml = new List<Color>();
    ...
    catch (Exception ex) { _ = ex.Log(); return -2; }
  }

  static void addIfN(Dictionary<int, int> mmcnt, int k) { if (mmcnt.ContainsKey(k)) mmcnt[k]++; else mmcnt.Add(k, 1); }
  static int dbgShow(Dictionary<int, int> mmcnt, int k) => mmcnt.ContainsKey(k) ? mmcnt[k] : 0;            //	Debug.WriteLine(mmcnt.ContainsKey(k) ? mmcnt[k] : 0, k.ToString("0####"));
}
