  ...
  void DrawUpDnLine(DateTime trgDate)
  {
    ...
    try
    {
      ...
      else
      {
        ...
        addUiElnt(.82 * _ah, 0, new Rectangle { Height = .18 * _ah, Width = Math.Abs(_dailyTimeSplit.WorkedFor.TotalDays) * _aw, Fill = new SolidColorBrush(Color.FromRgb(128, 160, 0)) });
        ...
        if (File.Exists(remoteLog))
        {
          ...
          addUiElnt(.0 * _ah, 0, new Rectangle { Height = .18 * _ah, Width = Math.Abs(remoteTimeSplit.WorkedFor.TotalDays) * _aw, Fill = new SolidColorBrush(Color.FromRgb(0, 100, 255)), ToolTip = "???" });
