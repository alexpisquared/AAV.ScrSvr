...
namespace AAV.SS
{
	public class BrowseForFolderDialog // Represents a common dialog box (Win32::SHBrowseForFolder()) that allows a user to select a folder.
	{
      ...
      BrowseInfo = new BROWSEINFOW
      {
		...
		Nullable<bool> PInvokeSHBrowseForFolder(Window owner)
		{
		...
		int BrowseEventHandler(IntPtr hwnd, MessageFromBrowser uMsg, IntPtr lParam, IntPtr lpData)
		{
			switch (uMsg)
			{
		...
		public class BROWSEINFOW
		{
			...
			/// <summary>
			/// A pointer to an item identifier list (PIDL) specifying the scrBounds of the root folder from which to start browsing. 
			/// </summary>
		...
		static extern IntPtr SHBrowseForFolderW([MarshalAs(UnmanagedType.LPStruct), In, Out] BROWSEINFOW bi);
		[DllImport("shell32.dll")]
